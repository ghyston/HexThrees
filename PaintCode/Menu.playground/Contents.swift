...
class GameScene: SKScene {
    ...
    private func createLine(pos: CGPoint) -> SKShapeNode {
        
        let w = 200
        let h = 20
        let c = 10
        
        let shape = SKShapeNode.init(rect: CGRect(x: -w/2, y: -h/2, width: w, height: h), cornerRadius: CGFloat(c))
        shape.fillColor = SKColor(rgb: 0xE0D688)
        shape.position = pos
        return shape
    }
    ...
    override func didMove(to view: SKView) {
        ...
        addChild(createLine(pos: CGPoint(x: 0, y: 0)))
        addChild(createLine(pos: CGPoint(x: 0, y: distance)))
        addChild(createLine(pos: CGPoint(x: 0, y: distance * 2)))
    }
